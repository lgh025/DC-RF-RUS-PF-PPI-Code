...
% load G:\RF_Class_C\data_n  
%   load G:\RF_Class_C\label_n  
%   load G:\RF_Class_C\traindata_p  
%   load G:\RF_Class_C\T_train_p  
% save(['../data/model1.mat'], 'model','data_n','label_n','traindata_p','T_train_p' );
load(['../data/model1.mat'], 'data_n','label_n','traindata_p','T_train_p' );

   P_data=traindata_p;%训练集正数据
   P_label=T_train_p;%训练集正标签
   N_data=data_n;%训练集负数据
   N_label=label_n ;%训练集负标签
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% result2 = [test_label{k}  M];


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% testlabels=result2(:,1);
% pred=result2(:,2);
%   %Confusion Matrix
...
%   err1
  ...
  order=test_number_store1{k};
   %saved
%   test_label_score1{k}=test_label{k}((M==1),:);
 test_data_score1{k}=test{k}((M==1),:);


 %deleted
%  test_label_score2{k}=test_label{k}((M==2),:);
 test_data_score2{k}=test{k}((M==2),:);

 %saved
%  test_label_scoreP1{k}=test_label{k}(testlabels(find(testlabels==1))== pred(find(testlabels==1)),:);
%  test_data_scoreP1{k}=test{k}(testlabels(find(testlabels==1))== pred(find(testlabels==1)),:);
% %  test_label_scoreN1{k}=test_label{k}(testlabels(find(testlabels==2))~= pred(find(testlabels==2)),:);
%  test_data_scoreN1{k}=test{k}(testlabels(find(testlabels==2))~= pred(find(testlabels==2)),:);
%  %deleted
%   test_label_scoreP2{k}=test_label{k}(testlabels(find(testlabels==1))~= pred(find(testlabels==1)),:);
%  test_data_scoreP2{k}=test{k}(testlabels(find(testlabels==1))~= pred(find(testlabels==1)),:);
%  test_label_scoreN2{k}=test_label{k}(testlabels(find(testlabels==2))== pred(find(testlabels==2)),:);
%  test_data_scoreN2{k}=test{k}(testlabels(find(testlabels==2))== pred(find(testlabels==2)),:);
%  
  %%%%%%%%%%%%%%%%%%%%%%%%UPDATE testdata,
testDATA1= data1(test_number_store1{k},:);
% testLABEL1= label1(test_number_store1{k},:);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
end
